		...
		while (remain > 0) {
			...
			if (!fileShards.empty()) {
				...
				for (auto it = last_shard.files.begin(); it != last_shard.files.end(); ++it) {
					totalsize += it->second.second - it->second.first + 1;
				}
				...
				if (totalsize / 1024.0 < map_size) {
					...
					cout <<endl<<endl<< "Extract_size updated to " << extract_size << "kB" <<endl<<endl<<endl;
					fileShards.pop_back();
					cur = last_shard;
				}
			}
			streampos start = fin.tellg();
			cout << "tellg of start : " << start << endl;
			...
			cout << "tellg of end after adjustment : " << end << endl;
